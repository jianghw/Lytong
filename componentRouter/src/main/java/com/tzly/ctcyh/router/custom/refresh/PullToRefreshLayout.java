...
public class PullToRefreshLayout extends RelativeLayout {
    ...
    // 刷新回调接口
    public IPullListener mIPullListener;
    // 下拉刷新过程监听器
    public IPullProcessListener mOnRefreshProcessListener;
    // 上拉加载更多过程监听器
    public IPullProcessListener mOnLoadmoreProcessListener;
     ...
     */
    public void setIPullListener(IPullListener pullListener) {
        mIPullListener = pullListener;
    }
    ...
    public View getPullRefreshableView() {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (childAt instanceof IPullToRefresh) {
                return childAt;
            }
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mPullDownEnable && !((IPullToRefresh) mPullRefreshable).canPullDown())
                    return false;
                if (mPullUpEnable && !((IPullToRefresh) mPullRefreshable).canPullUp())
                    return false;
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (null != mOnRefreshProcessListener && pullDownY > 0) {
                    if (!mPreparedPullDown) {
                        ...
                        mOnRefreshProcessListener.onPrepare(mRefreshView, IPullProcessListener.REFRESH);
                    }
                    mOnRefreshProcessListener.onPull(mRefreshView, pullDownY, IPullProcessListener.REFRESH);
                }
                //git 上拉 先不用
                if (null != mOnLoadmoreProcessListener && pullUpY < 0) {
                    if (!mPreparedPullUp) {
                        mPreparedPullUp = true;
                        mOnLoadmoreProcessListener.onPrepare(mUploadMoreView, IPullProcessListener.LOADMORE);
                    }
                    mOnLoadmoreProcessListener.onPull(mUploadMoreView, pullUpY, IPullProcessListener.LOADMORE);
                }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mViewState == RELEASE_TO_REFRESH) {
                    ...
                    // 刷新操作
                    if (mIPullListener != null) mIPullListener.onRefresh(this);
                } else if (mViewState == RELEASE_TO_LOAD) {
                    changeState(LOADING);
                    // 加载操作
                    if (mIPullListener != null) mIPullListener.onLoadMore(this);
                }
                hide();
            default:
                break;
    ...
    protected void slidingDistanceOperation(MotionEvent event) {
        ...
        if (pullDownY > 0 ||
                (((IPullToRefresh) mPullRefreshable).canPullDown()
        ...
        } else if (pullUpY < 0 || (((IPullToRefresh) mPullRefreshable).canPullUp()
        ...
        switch (mViewState) {
            ...
            case RELEASE_TO_REFRESH://释放刷新
                if (null != mOnRefreshProcessListener) {
                    mOnRefreshProcessListener.onStart(mRefreshView, IPullProcessListener.REFRESH);
                }
            ...
            case REFRESHING://正在刷新
                if (null != mOnRefreshProcessListener) {
                    mOnRefreshProcessListener.onHandling(mRefreshView, IPullProcessListener.REFRESH);
                }
            ...
            case RELEASE_TO_LOAD://释放加载
                if (null != mOnLoadmoreProcessListener) {
                    mOnLoadmoreProcessListener.onStart(mUploadMoreView, IPullProcessListener.LOADMORE);
                }
                if (null == customUploadMoreView) {
                    if (pullUpArrows == null) return;
            ...
            case LOADING:// 正在加载状态
                if (null != mOnLoadmoreProcessListener) {
                    mOnLoadmoreProcessListener.onHandling(mUploadMoreView, IPullProcessListener.LOADMORE);
                }
                if (null == customUploadMoreView) {
                    if (pullUpArrows == null) return;
                    pullUpArrows.clearAnimation();
    ...
    public void refreshFinish(final int refreshResult) {
        ...
        if (null != mOnRefreshProcessListener) {
            mOnRefreshProcessListener.onFinish(mRefreshView, IPullProcessListener.REFRESH);
        }
        if (null == customRefreshView) {
            refreshingView.clearAnimation();
            refreshingView.setVisibility(View.INVISIBLE);
    ...
    public void uploadMoreFinish(int refreshResult) {
        if (null != mOnLoadmoreProcessListener) {
            mOnLoadmoreProcessListener.onFinish(mUploadMoreView, IPullProcessListener.LOADMORE);
        }
        if (null == customUploadMoreView) {
            loadingView.clearAnimation();
            loadingView.setVisibility(View.INVISIBLE);
            ...
            case SUCCEED:// 加载成功
                if (null == customUploadMoreView) {
                    ...
                    if (loadStateImageView instanceof ImageView)
                        ((ImageView) loadStateImageView).setImageResource(R.mipmap.ic_load_more_succeed);
                }
                break;
            case FAIL:
            default:
                ...
                if (null == customUploadMoreView) {
                    ...
                    loadStateImageView.setVisibility(View.VISIBLE);
                    if (loadStateImageView instanceof ImageView)
                        ((ImageView) loadStateImageView).setImageResource(R.mipmap.ic_load_more_failed);
                }
                break;
        }
        if (pullUpY < 0) {
     ...
     * 设置下拉刷新过程监听器
     */
    public void setOnRefreshProcessListener(IPullProcessListener iPullProcessListener) {
        mOnRefreshProcessListener = iPullProcessListener;
    }

    /**
     * 设置上拉加载更多过程监听器
     */
    public void setOnLoadmoreProcessListener(IPullProcessListener iPullProcessListener) {
        mOnLoadmoreProcessListener = iPullProcessListener;
    }
