...
//            setBadgeOfMIUI(context, count, iconResId);
    ...
    private static void sendToXiaoMi(Context context, int count) {
        try {
            ...
            field.set(miuiNotification, String.valueOf(count == 0 ? "" : count)); // 设置信息数-->这种发送必须是miui6才行
        } catch (Exception e) {
            e.printStackTrace();
            // miui 6之前的版本
    ...
    private static void setBadgeOfVIVO(Context context, int count) {
        try {
            ...
            String launchClassName = context.getPackageManager()
                    .getLaunchIntentForPackage(context.getPackageName()).getComponent().getClassName();
    ...
    public static void setBadgeOfMadMode(Context context,
