...
/***/ function(module, exports, __webpack_require__) {
	...
	$(function(){
	    ...
	    $(document).on('pageInit', '#shear_index', function () {
	        ...
	        var signIn_days = '';//连续签到天数
	        var isBindCard=window.CTTX?window.CTTX.getBindCardStatus():1;   //是否绑卡 0-已绑 1-未绑
	        var userId=window.CTTX?window.CTTX.getUserId():"00335517717406260"; //获取用户id
	        var activityFirst_json = {"usrnum":userId}; //请求参数
	        ...
	        $.ajax({
	            ...
	            success: function (msg) {
	                if(!msg.data){
	                    window.location.href='../pages/peccancy.html';
	                }
	                //console.log(msg);
	                signIn_days = msg.data.continuitySign;
	                if(!(signIn_days >= 0)){    //签到天数
	                    $('#signIn_days').html('+0');
	                }else{
	                    $('#signIn_days').html('+'+signIn_days);
	                }
	                $('#noViolation_count').html(msg.data.notPeccancy); //无违章人数
	                $('#user_information').html(msg.data.id);   //车牌号
	                $('#user_information_all').html(msg.data.ranking);  //我的排名
	                $('#user_integral').html(msg.data.points);  //我的积分
	                if(msg.data.isSign){    //是否已签到
	                    $("#sign_In").html("已签到");
	                }else{
	                    $("#sign_In").html("签到");
	                }
	        ...
	        //活动天数计算，2017-3-17日开始
	        var today = new Date();
	        var beginDate = new Date('2017-3-17');
	        var dateCount = (today.getTime() - beginDate.getTime()) / (24 * 60 * 60 * 1000);
	        if(dateCount>=0){
	            $("#sign_count").html(parseInt(dateCount));
	        }else{
	            $("#sign_count").html("0");
	        }
	        ...
	        $.ajax({
	            ...
	            success: function (msg) {
	                ...
	                //前三名添加图标
	               $(".rank_list").find(".font_red")[0].innerHTML='<img src="imgs/1@2x.png">';
	               $(".rank_list").find(".font_red")[1].innerHTML='<img src="imgs/2@2x.png">';
	               $(".rank_list").find(".font_red")[2].innerHTML='<img src="imgs/3@2x.png">';
	               var len=msg.data.length;
	                $(".rank_list").find(".font_red")[len].innerHTML='<img src="imgs/1@2x.png">';
	               $(".rank_list").find(".font_red")[len+1].innerHTML='<img src="imgs/2@2x.png">';
	               $(".rank_list").find(".font_red")[len+2].innerHTML='<img src="imgs/3@2x.png">';
	                //循环显示
	        ...
	        //判断是否已签到-通过localStorage记录的上次签到时间判断
	        // var lastSign = localStorage.getItem("signDay");
	        // var hasSign=parseInt((today-lastSign)/ (24 * 60 * 60 * 1000));
	        // if(hasSign<1){
	        //     $("#sign_In").html("已签到");
	        // }
	            ...
	            if(_this_html == '签到'){
	                $.ajax({
	                    ...
	                    success: function (msg) {
	                        ...
	                        localStorage.setItem("signDay", nowTime);
	                        _this.html('已签到');
	        ...
	        //根据是否绑卡改变样式
	        if(!isBindCard){
	            document.getElementById('act-bindCard').src='img/images/icon_bangka_no@2x.png';
	            document.getElementById('act-bindCardText').innerHTML='已绑卡';   
	        }
	        

	        //事件绑定
	        //绑卡
	        $("#act-bindCard").on("click",function(){
	                if(isBindCard){
	                    window.CTTX.bindCard();
	                }else{
	                    $.toast("已绑卡");
	                }
	        })
	        //加油
	        $("#act-addOil").on("click",function(){
	            if(window.CTTX){
	                window.CTTX.addOil();
	            }
	        ...
	        //交友
	        $("#act-shareActivity").on("click",function(){
	            if(window.CTTX){
	                window.CTTX.shareActivity();
	            }
	            
	        })
	        //积分明细
	        $("#activity_todetal").on("click",function(){
	             window.location.href='pages/rule.html';
	        })
	    ...
	    $(document).on('pageInit', '#page_rule', function () {
	        ...
	        //积分明细接口
	        var userId=window.CTTX?window.CTTX.getUserId():"00335517717406260"; //获取用户id
	        var integralDetail_json = {"usrnum":userId};
	    ...
	    $(document).on('pageInit','#page_pecc',function(){
	        $.ajax({
	            url: "http://139.196.183.121:8081/february/activityCount",
	            type: 'POST',
	            data: '',
	            ...
	            success: function (msg) {
	                console.log(msg);
	                $('#pecc_activityCount').html(msg.data.activityCount);
	                $('#pecc_noViolationCount').html(msg.data.notPeccancy);
	            },
	            error: function (msg) {
	                alert("请求申请信息超时");
	            }
