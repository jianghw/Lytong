...
//                                    .setBackgroundResource(R.mipmap.refresh_succeed);
                        ...
                        if (pullDownY > 0) {
                            // 刷新结果停留1秒
                            new RemainHandler(PullToRefreshLayout.this).sendEmptyMessageDelayed(0, 0);
                        } else {
                            changeState(DONE);
                            hide();
                        }
...
//                    refreshStateTextView.setText(R.string.refreshing);
    ...
    private boolean isRefreshViewScroll(int deltaY) {
        ...
        if (pullableView != null) {
            // 子scroll view滑动到最顶端
            if (deltaY > 0 && pullableView.getScrollY() == 0) {
                return true;
            } else if (deltaY < 0 && pullableView.getMeasuredHeight() <= getHeight()
                    + pullableView.getScrollY()) {
                return true;
            }
        }
        return false;
    ...
    static class UpdateHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (null != layout) {
                ...
                layout.mMoveSpeed = (float) (18 + 5 * Math.tan(Math.PI / 2
