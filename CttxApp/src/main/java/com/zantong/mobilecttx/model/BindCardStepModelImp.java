 ...
 */
public class BindCardStepModelImp implements SimpleModel {
    private Subscriber mSubscriber;
    @Override
    public void loadUpdate(final OnLoadServiceBackUI listener, String msg, final int index) {
        switch (index){
            case 1:
                mSubscriber = new Subscriber<BindCardStepBean>() {
                    @Override
                    ...
                    @Override
                    public void onNext(BindCardStepBean mBindCardStepBean) {
                        Log.e("why", mBindCardStepBean+"aaa");
                        listener.onSuccess(mBindCardStepBean, index);
                    }

                };
                APPHttpClient.getInstance().getCTTXHttpPOSTInterface().loadBindCard(msg)
