...
public class ViolationDetails extends BaseJxActivity implements ModelView {
    ...
    private void initText(ViolationDetailsBean violationDetailsBean) {
        ...
        for (int i = 0; i < LoginData.getInstance().payData.size(); i++) {
            if (violationNumber.equals(LoginData.getInstance().payData.get(i).getCarnum())) {
                isPagCar = true;
                break;
            } else {
                isPagCar = false;
        ...
        if ("0".equals(mRspInfo.getProcessste())
            ...
            if ("1".equals(bitNumber) || "2".equals(bitNumber)) {//是否处罚决定书
                ...
                mCommitType = 1;
            } else if (isPagCar || LoginData.getInstance().payData.size() < 2) {
                mNextBtn.setEnabled(true);
                mNextBtn.setText("违章缴费");
                mCommitType = 0;
            } else {
    ...
    protected void submitData() {
        switch (mCommitType) {
            case 0:
                LoginData.getInstance().mHashMap.clear();
