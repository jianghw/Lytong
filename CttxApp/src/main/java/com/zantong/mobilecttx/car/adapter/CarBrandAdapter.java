...
public class CarBrandAdapter extends BaseAdapter implements SectionIndexer {
	private List<CarBrandBean> list = null;
	private Context mContext;

	public CarBrandAdapter(Context mContext, List<CarBrandBean> list) {
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		if(position == getPositionForSection(section)){
			...
			viewHolder.tvLetter.setText(mContent.getBrandPinYin());
		}else{
			viewHolder.tvLetter.setVisibility(View.GONE);
		}
		viewHolder.tvTitle.setText(this.list.get(position).getBrandName());
		return view;
	}
	...
	public int getSectionForPosition(int position) {
		return list.get(position).getBrandPinYin().charAt(0);
	}
	...
	public int getPositionForSection(int section) {
		for (int i = 0; i < getCount(); i++) {
			String sortStr = list.get(i).getBrandPinYin();
